...
namespace FSMlearning
{
	struct gs_node_t
	{
		...
		gs_node_t(sequence_in_t seq, const input_t &numInputs) : accessSequence(move(seq)), state(NULL_STATE), succ(numInputs),
	...
	static bool areNodesDifferent(const shared_ptr<gs_node_t> &first, const shared_ptr<gs_node_t> &second)
	{
		if ((first->stateOutput != second->stateOutput) &&
			(first->stateOutput != DEFAULT_OUTPUT) && (second->stateOutput != DEFAULT_OUTPUT))
			return true;
						   ...
						   const unique_ptr<DFSM> &conjecture, const unique_ptr<Teacher> &teacher)
	{
		...
		for (state_t state = 0; state < end; state++)
		{
			for (input_t i = 0; i < conjecture->getNumberOfInputs(); i++)
			{
				...
				{ //if (node->consistentStates.size() == 1) {
					checkNumberOfOutputs(teacher, conjecture);
	...
	static void query(shared_ptr<gs_node_t> node, const sequence_in_t &sequence, const unique_ptr<Teacher> &teacher, bool setStateOutput)
	{
		...
		for (auto &input : sequence)
		{
			...
			if (!teacher->isProvidedOnlyMQ())
			{
				if (setStateOutput)
					...
					node->incomingOutput = output.front();
				output.pop_front();
			}
			else
			{
		...
		if (teacher->isProvidedOnlyMQ())
		{
			if ((sequence.back() == STOUT_INPUT) || setStateOutput)
				node->stateOutput = output.back();
			else
				node->incomingOutput = output.back();
			node->counter--;
		}
	}
		...
		for (state_t state = 0; state < stateNodes.size(); state++)
		{
			for (input_t i = 0; i < conjecture->getNumberOfInputs(); i++)
			{
				...
				while (node->consistentStates.size() > 1)
				{
					...
					{
						if (getLastOutput(node, *it, !conjecture->isOutputTransition()) != DEFAULT_OUTPUT)
							continue;
						...
						if (teacher->isProvidedOnlyMQ())
						{
							...
							{
								if (distinguishedNum < p.second)
									distinguishedNum = p.second;
							}
						}
						else
						{
							...
							if (withoutSeq == node->consistentStates.size())
								continue;
								  ...
								  const unique_ptr<DFSM> &conjecture, const unique_ptr<Teacher> &teacher, size_t remainingSeq)
	{
		...
		while (numQueries > 0)
		{
			...
			if (getLastOutput(node->consistentStates.front(), distSequences[seqIdx], !conjecture->isOutputTransition()) == DEFAULT_OUTPUT)
			{
				...
				if (teacher->isProvidedOnlyMQ())
					remainingSeq--; // some state's next node was extended
			}
			bool distinguished = (teacher->isProvidedOnlyMQ() ? (getLastOutput(node, distSequences[seqIdx], !conjecture->isOutputTransition()) !=
							   ...
							   function<bool(const unique_ptr<DFSM> &conjecture)> provideTentativeModel, bool isEQallowed)
	{
		...
		{
			if (!provideTentativeModel(conjecture))
				return conjecture;
		}
		...
		while (len <= maxDistinguishingLength)
		{
			...
			if (10 * totalApplied >= 9 * totalDistSeq)
			{ // totalApplied needs to be at least 90 % of totalDistSeq
				len++;
