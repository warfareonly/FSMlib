...
namespace FSMlearning
{
	...
	static void addTransition(const unique_ptr<DFSM> &conjecture, const state_t &from, const input_t &input, const state_t &to, ObservationTable &ot)
	{
		...
		if (conjecture->isOutputTransition())
		{
			if (conjecture->getNextState(from, input) == NULL_STATE)
			{
				...
				output = conjecture->getOutput(from, input);
		}
	...
	void addSuffixToE_binarySearch(const sequence_in_t &ce, ObservationTable &ot, const unique_ptr<Teacher> &teacher)
	{
		...
		if (!sameOutput || (suffix.front() == STOUT_INPUT))
			suffix.pop_front();
	...
	static bool isSemanticSuffixClosed(ObservationTable &ot)
	{
		...
		if (distSeq.empty() || (find(ot.E.begin(), ot.E.end(), distSeq) != ot.E.end()))
		{ // a cyclic undistingushable block
			distSeq.clear();
						   ...
						   function<bool(const unique_ptr<DFSM> &conjecture)> provideTentativeModel, bool checkPreviousCE, bool checkSemanticSuffixClosedness)
	{
		...
		while (unlearned)
		{
			...
			{ // EQ -> CE or stop
				...
				{
					if (conjecture->isOutputState() && !conjecture->isOutputTransition())
					{ // Moore and DFA
						sequence_in_t newCE;
						for (auto &input : ce)
						{
					...
					if (ssize != ot.S.size())
					{
						...
						{ // increasing order of prefixes is supposed
