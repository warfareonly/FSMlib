...
namespace FSMlearning
{
	struct ot_node_t
	{
		...
		ot_node_t(vector<input_t> inputs) : incomingOutput(DEFAULT_OUTPUT), state(NULL_STATE), stateOutput(DEFAULT_OUTPUT),
		...
		ot_node_t(const shared_ptr<ot_node_t> &parent, input_t input, input_t idx,
				  output_t transitionOutput, output_t stateOutput, vector<input_t> inputs) : accessSequence(parent->accessSequence), incomingOutput(transitionOutput), state(NULL_STATE),
	...
	{
		if ((node->nextInputs.size() > input) && (node->nextInputs[input] == input))
			return input;
	...
	{
		return (n1->stateOutput != n2->stateOutput) || (n1->incomingOutput != n2->incomingOutput) || (n1->nextInputs != n2->nextInputs) || (n2->consistentNodes.find(n1.get()) == n2->consistentNodes.end());
	}
	...
	static void addADS(shared_ptr<ot_node_t> node, ObservationTree &ot)
	{
		...
		do
		{
			...
			if (adsDec->initialStates.size() == 1)
			{ // new node
				break;
			}
			if (node->distInputIdx == STOUT_INPUT)
			...
			if (!ads)
			{
				...
				for (state_t sIdx = 0; sIdx < adsDec->initialStates.size() - 1; sIdx++)
				{ // all but the current refState
		...
		if (idx != STOUT_INPUT)
		{
			for (auto cnIt = node->consistentNodes.begin(); cnIt != node->consistentNodes.end();)
			{
				if ((*cnIt)->next[idx] && (areNextNodesDifferent(node->next[idx], (*cnIt)->next[idx])))
				{
					...
					{ // is this possible? there needs to be the same path from the consistent node and the path was not from this stateNode
	...
	static void query(const shared_ptr<ot_node_t> &node, input_t idx, ObservationTree &ot, const unique_ptr<Teacher> &teacher)
	{
		...
		if (!teacher->isProvidedOnlyMQ() && (ot.conjecture->getType() == TYPE_DFSM))
		{
			...
			auto output = (ot.bbNode == node) ? teacher->outputQuery(suffix) : teacher->resetAndOutputQueryOnSuffix(node->accessSequence, suffix);
			transitionOutput = output.front();
			stateOutput = output.back();
		}
		else
		{
			transitionOutput = (ot.bbNode == node) ? teacher->outputQuery(input) : teacher->resetAndOutputQueryOnSuffix(node->accessSequence, input);
			if (ot.conjecture->getType() == TYPE_DFSM)
			{
				stateOutput = teacher->outputQuery(STOUT_INPUT);
			}
			else if (!ot.conjecture->isOutputTransition())
			{ // Moore, DFA
	...
	static bool checkResponse(const shared_ptr<ot_node_t> &node, input_t idx, ObservationTree &ot)
	{
		...
		{
			if (ot.conjecture->getOutput(ot.conjecture->getNextState(*(node->refStates.begin()), node->nextInputs[idx]), STOUT_INPUT) != node->next[idx]->stateOutput)
				return false;
		}
		return true;
	}
							  ...
							  seq_len_t &totalLen, seq_len_t currLength)
	{
		...
		for (auto p : next)
		{
			...
			{ // choose the best next input for ADS
				frac_t bestVal(0, 1);
	...
	static shared_ptr<ot_ads_t> chooseADS(const shared_ptr<ot_node_t> &node, state_t expectedState, const ObservationTree &ot)
	{
		...
		for (auto &currADS : adsVec)
		{
			...
			if (fracLess(maxVal, val) || (!fracLess(val, maxVal) && ((minLen > totalLen) || ((minLen == totalLen) && (currADS->input == prevInput)))))
			{
				maxVal = val;
				minLen = totalLen;
				ads = currADS;
											  ...
											  function<bool(const unique_ptr<DFSM> &conjecture)> provideTentativeModel)
	{
		...
		while (!ot.uncheckedNodes.empty())
		{
			...
			for (input_t i = 0; i < node->nextInputs.size(); i++)
			{
				if (!node->next[i])
				{
					...
					if (node->state != NULL_STATE)
					{
						...
						if (ot.conjecture->isOutputState() && nextNode->refStates.empty())
						{ // new state
							checkAndQueryNext(nextNode, ot);
							nextNode = nullptr;
						}
					}
					else if (!checkResponse(node, i, ot))
					{ // new state
						checkAndQueryNext(nextNode, ot);
						nextNode = nullptr;
					}
			...
			{
				if (!provideTentativeModel(ot.conjecture))
					break;
			}
		}
