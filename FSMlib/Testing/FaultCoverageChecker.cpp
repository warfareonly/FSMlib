...
namespace FSMtesting
{
	namespace FaultCoverageChecker
	{
		...
		struct StateVariable
		{
			...
			StateVariable(input_t input, state_t state, output_t output, input_t P, state_t MaxStates) : prevInput(input), state(state), stateOutput(output)
			{
				next.resize(P);
			...
			StateVariable(const StateVariable &sv) : varIdx(sv.varIdx), instance(sv.instance), state(sv.state), prev(sv.prev),
		...
		struct LogItemFC
		{
			...
			LogItemFC(state_t from, input_t input, state_t to, output_t output) : from(from), input(input), output(output), to(to)
			{
			}
		};
		...
		static vector<shared_ptr<StateVariable>> initVar(const unique_ptr<DFSM> &fsm, const sequence_set_t &TS, int extraStates)
		{
			...
			for (auto &seq : TS)
			{
				...
				for (auto &input : seq)
				{
					...
					actStateVar->transitionOutput[input] = (fsm->isOutputTransition()) ? fsm->getOutput(actStateVar->state, input) : DEFAULT_OUTPUT;
			...
			for (auto j : var[idx]->different)
			{
				if (var[j])
				{
					if (var[j]->instance == NULL_STATE)
					{
						...
						{
							if (!instantiate(var[j], var, refStates, instantiated))
								return false;
						}
					}
		...
		static bool reduceDomains(vector<shared_ptr<StateVariable>> &var, vector<state_t> &refStates, stack<state_t> &instantiated)
		{
			...
			{
				if (var[i]->instance != NULL_STATE)
					continue; // a ref state
				...
				else
				{
					...
					{
						if (!instantiate(var[i], var, refStates, instantiated))
							return false;
					}
				}
			}
							   ...
							   vector<shared_ptr<StateVariable>> &var, vector<state_t> &refStates, stack<state_t> &instantiated)
		{
			...
			{
				if (!instantiate(toNode, var, refStates, instantiated))
					return false;
			}
			else if (fromNode->instance == NULL_STATE)
			{
				if (toNode->domain.count() == 1)
				{
					...
					if (!instantiate(fromNode, var, refStates, instantiated))
						return false;
				}
					...
					if (toNode->next[i])
					{
						...
						if (fromNode->next[i]->isRefState)
						{
							...
							if (toNode->next[i]->instance == NULL_STATE)
							{
								...
								if (!instantiate(toNode->next[i], var, refStates, instantiated))
									return false;
							}
						...
						if (!mergeNodes(fromNode->next[i], toNode->next[i], var, refStates, instantiated))
							return false;
					}
					else
					{
		...
		static bool processInstantiated(vector<shared_ptr<StateVariable>> &var, vector<state_t> &refStates, stack<state_t> &instantiated)
		{
			...
			while (!instantiated.empty())
			{
				...
				instantiated.pop();
				if (!consistent || (!var[i]) || (var[i]->isRefState))
					continue;
			...
			for (input_t input = 0; input < node->next.size(); input++)
			{
				...
				if (nextState != NULL_STATE)
				{
					...
					if (insNS == NULL_STATE)
					{
						...
						if (!setTransitions(nodeNS, partial, log, var, refStates, instantiated))
							return false;
					}
					if (outputTransition != outTranNS)
					{ // set transition output
