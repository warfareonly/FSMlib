...
namespace FSMtesting
{
	struct TestNodeH
	{
		...
		TestNodeH(state_t state, output_t stateOutput, output_t inOutput) : incomingOutput(inOutput), stateOutput(stateOutput), state(state)
		{
		}
	};
			...
			if (sIt1 != n1->next.end())
			{
				if (sIt2 != n2->next.end())
				{
					if ((sIt1->second->incomingOutput != sIt2->second->incomingOutput) ||
						(sIt1->second->stateOutput != sIt2->second->stateOutput))
						return 0; //distinguished
					if (sIt1->second->state == sIt2->second->state)
						continue;
										 ...
										 const unique_ptr<DFSM> &fsm, const vector<LinkCell> &sepSeq)
	{
		...
		if (it == n1->next.end())
		{
			...
			else
			{
				...
				outputTransition1 = (fsm->isOutputTransition()) ? fsm->getOutput(n1->state, input) : DEFAULT_OUTPUT;
			}
		...
		if (it == n2->next.end())
		{
			...
			else
			{
				...
				outputTransition2 = (fsm->isOutputTransition()) ? fsm->getOutput(n2->state, input) : DEFAULT_OUTPUT;
			}
	...
	static void distinguish(const shared_ptr<TestNodeH> &n1, const shared_ptr<TestNodeH> &n2,
							const unique_ptr<DFSM> &fsm, const vector<LinkCell> &sepSeq)
	{
		if ((n1->state == n2->state) || (n1->stateOutput != n2->stateOutput) || (getMinLenToDistinguish(n1, n2, sepSeq, fsm->getNumberOfStates(), fsm->getNumberOfInputs()) == 0))
			return;
