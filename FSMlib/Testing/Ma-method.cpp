			...
			else if (verifiedState[currState] > 0)
			{
				for (input_t input = 0; input < P; input++)
				{
					if (!verifiedTransition[currState][input])
					{
						...
						if (fsm->isOutputState() && (E[nextState].front() != STOUT_INPUT))
							CS.push_back(STOUT_INPUT);
