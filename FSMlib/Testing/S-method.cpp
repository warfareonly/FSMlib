...
namespace FSMtesting
{
	...
	static bool areNodesDifferent(const shared_ptr<OTreeNode> &n1, const shared_ptr<OTreeNode> &n2)
	{
		...
		{
			if ((n1->next[i]) && (n2->next[i]) && ((n1->next[i]->incomingOutput != n2->next[i]->incomingOutput) || areNodesDifferent(n1->next[i], n2->next[i])))
				return true;
		}
		return false;
	}
	...
	{
		if ((cn1 == cn2) || (cn1->state == cn2->state))
			return false;
		...
		if (cn1->isRN || cn2->isRN)
		{
			if ((cn1->isRN && cn2->isRN) || // different RNs are distinguished
				!cn1->domain.count(cn2.get()))
				return true;
			if (cn1->isRN)
			{
				if (!closed.emplace(cn1->state, cn2.get()).second)
					return false;
			}
			else
			{
				if (!closed.emplace(cn2->state, cn1.get()).second)
					return false;
			}
		}
		else if (noES && isIntersectionEmpty(cn1->domain, cn2->domain))
			return true;
		for (input_t i = 0; i < cn1->next.size(); i++)
		{
			if ((cn1->next[i]) && (cn2->next[i]))
			{
				...
				if (((*it1)->next[i]->incomingOutput != (*it2)->next[i]->incomingOutput) || areConvergentNodesDistinguished(cn1->next[i], cn2->next[i], noES, closed))
					return true;
			}
		}
		return false;
	...
	{
		if (node->stateOutput != cn->convergent.front()->stateOutput)
			return true;
		...
		if (cn->isRN || cn1->isRN)
		{
			...
			else if (!cn1->domain.count(cn))
				return true;
		}
		else if (noES && isIntersectionEmpty(cn1->domain, cn->domain))
			return true;
		if ((node->lastQueriedInput == STOUT_INPUT) || (!cn->next[node->lastQueriedInput]))
			return false;
		...
		return ((node->next[input]->incomingOutput != (*it)->next[input]->incomingOutput) || areDistinguishedUnder(node->next[input], cn->next[input].get(), noES));
	}
							...
							const OTree &ot, const unique_ptr<DFSM> &fsm)
	{
		...
		for (auto nodeIt = cn->convergent.begin(); nodeIt != cn->convergent.end(); ++nodeIt)
		{
			...
			if (node)
				return; // sequence already queried
		}
		if ((maxLen == 0) && (!cn->isRN || (cn->state != 0)))
		{
			...
			while (!fifo.empty())
			{
				...
				for (auto &n : p.first->convergent)
				{
					...
					if (!p.first->isRN)
						break; // only RN can have different predecessors
				}
			}
										  ...
										  list<shared_ptr<OTreeNode>> &leaves)
	{
		...
		for (input_t input = 0; input < cn->next.size(); input++)
		{
			if (node->next[input])
			{
				if (!node->next[input]->convergentNode.lock())
				{ // not a state node
		...
		while (!domain.empty())
		{
			...
			for (auto dIt = domain.begin(); dIt != domain.end();)
			{
				...
				if (!cn->domain.count((*dIt).get()) &&
					((find(nodes.begin(), nodes.end(), *dIt) == nodes.end()) || areConvergentNodesDistinguished(cn, *dIt, ot.es == 0, closed)))
				{
					dIt = domain.erase(dIt);
				}
				else
	...
	sequence_set_t S_method_ext(const unique_ptr<DFSM> &fsm, OTree &ot, StateCharacterization &sc)
	{
		...
		transitions.sort([](const tran_t &t1, const tran_t &t2)
						 { return (get<0>(t1)->convergent.front()->accessSequence.size() + get<2>(t1)->convergent.front()->accessSequence.size()) < (get<0>(t2)->convergent.front()->accessSequence.size() + get<2>(t2)->convergent.front()->accessSequence.size()); });
		...
		while (!transitions.empty())
		{
			...
			if (startCN->next[input] && (startCN->next[input] == nextStateCN))
				continue; // no ES and already verified
			...
			if (proveConvergence && (startCN->next[input] != nextStateCN))
			{ // already merged in case of no ES
