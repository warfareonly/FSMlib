...
namespace FSMtesting
{
	...
	struct TestNodeSPYH
	{
		...
		TestNodeSPYH(seq_len_t accessSeqLen, output_t inOutput, output_t stateOutput, state_t state, input_t numberOfInputs) : depth(accessSeqLen), incomingOutput(inOutput), stateOutput(stateOutput), state(state), next(numberOfInputs)
		{
		}
	};
											...
											const vector<LinkCell> &sepSeq)
	{
		...
		if (!hasLeaf1)
			minVal += cn1->convergent.front()->depth;
		if (!hasLeaf2)
			minVal += cn2->convergent.front()->depth;
		...
		for (input_t i = 0; i < cn1->next.size(); i++)
		{
			if (cn1->next[i])
			{
				if (cn2->next[i])
				{
					...
					if (cn1->next[i]->state == cn2->next[i]->state)
						continue;
					...
					if (est != seq_len_t(-1))
					{
						if (est != 1)
						{
							if (hasLeaf1)
								...
								est += cn1->next[i]->convergent.front()->depth;
						}
						if (!hasLeaf2)
							est += cn2->convergent.front()->depth;
						if (minVal > est)
						{
							minVal = est;
							input = i;
				...
				if (est != seq_len_t(-1))
				{
					if (est != 1)
					{
						if (hasLeaf2)
							...
							est += cn2->next[i]->convergent.front()->depth;
					}
					if (!hasLeaf1)
						est += cn1->convergent.front()->depth;
					if (minVal > est)
					{
						minVal = est;
						input = i;
							...
							const vector<LinkCell> &sepSeq, const unique_ptr<DFSM> &fsm)
	{
		...
		{
			if ((n != cn) && (n->state != cn->state) && (n->convergent.front()->stateOutput == cn->convergent.front()->stateOutput) && (getMinLenToDistinguish(cn, n, sepSeq) > 0))
			{
				auto cn1 = cn;
				auto cn2 = n;
				sequence_in_t seq;
	...
	sequence_set_t SPYH_method(const unique_ptr<DFSM> &fsm, int extraStates)
	{
		...
		transitions.sort([](const tran_t &t1, const tran_t &t2)
						 { return (get<0>(t1)->convergent.front()->depth + get<2>(t1)->convergent.front()->depth) < (get<0>(t2)->convergent.front()->depth + get<2>(t2)->convergent.front()->depth); });
