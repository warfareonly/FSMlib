...
namespace FSMtesting
{
	sequence_set_t Wp_method(const unique_ptr<DFSM> &fsm, int extraStates, const vector<sequence_set_t> &H)
	{
		...
		if (H.empty())
			SCSetsTmp = getStatesCharacterizingSets(fsm, getStatePairsShortestSeparatingSequences, true, reduceSCSet_LS_SL);
		else
			SCSets = H;
		...
		for (const auto &trSeq : stateCover)
		{
			for (const auto &extSeq : traversalSet)
			{
				...
				if (extSeq.size() == extraStates)
				{ // check outcoming transitions
